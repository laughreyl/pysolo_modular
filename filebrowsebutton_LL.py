...
class FileBrowseButton(wx.Panel):
    ...
    def createDialog(self, parent, id, pos, size, style, name):
        ...
        box.Add(self.textControl, 4, wx.LEFT | wx.CENTER, 5)

        self.browseButton = self.createBrowseButton()
        box.Add(self.browseButton, 1, wx.LEFT | wx.CENTER, 5)
    ...
    def OnBrowse(self, event=None):
        ...
        current_dir, current_file = os.path.split(current)
        if os.path.isdir(self.startDirectory):          # use startDirectory from call parameters
            directory = self.startDirectory
        elif os.path.isdir(current_dir):         # if textctrl directory is valid, use current textctrl directory
            directory = current_dir
        else:                                   # if textctrl directory is invalid, use cwd
            directory = os.getcwd()

        if self.initialValue != '':
            current = self.initialValue             # regardless, use the initial value for the filename if it's given
        else:
            current = current_file                  # otherwise use the current filename
